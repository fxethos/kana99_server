...
let destinationKey = new solanaWeb3.PublicKey("3hdEvz5Sj6GXXPfQXLFQQTLoAejLbcomWhpQpRNKSJ47")
...
const establishPayer = async () => {
  ...
  const payment = await connection.requestAirdrop(payer.publicKey, 1000000000);
  console.log(`airdropped payer ${payer.publicKey} and the signature is ${payment}`);
...
const checkProgram = async () => {
  ...
  if (greetedAccount === null) {
    ...
    console.log("Current payer:", payer.publicKey.toBase58(), " Balance:", await connection.getBalance(payer.publicKey));
...
const sendPayouts = async (addressarray) => {
  ...
  addressarray.find(element => {
    //console.log("element", element.wallet_address)
    payout_accounts.push({ "pubkey": new solanaWeb3.PublicKey(element.wallet_address), isSigner: false, isWritable: true })

  })
  // for (let i = 0; i < requiredAccounts.length; i++) {
  //   console.log(addressarray)
  //   payout_accounts.push({"pubkey" : new solanaWeb3.PublicKey(addressarray[i].wallet_address),isSigner: false, isWritable: true})

  // }
  //console.log("payout accounts" ,payout_accounts)
  let payout_keys = [...source_keys, ...payout_accounts]
  //console.log(payout_keys)
  //console.log(payout_keys.length-1)
  let payout_Amount = new PayoutAmount()
  payout_Amount.amount = addressarray[0].contest_value
  ...
  ).then(() => { console.log('payouts ') }).catch((e) => { console.log(e) });
